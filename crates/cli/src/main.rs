...
fn translate_files(
    ...
    // Incremental caching is skipped in print mode to avoid unexpected filesystem side effects.
    let use_cache = incremental && !print;
    ...
    for java_file in java_files {
        if use_cache && !cache.is_stale(java_file)? {
    ...
    info!("translation complete: {translated} translated, {skipped} skipped (unchanged), {failed} failed");
    if failed > 0 {
        anyhow::bail!("{failed} file(s) failed to translate");
    }
    Ok(())
}
